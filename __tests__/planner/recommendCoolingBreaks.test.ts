// Feature: heatshield-planner, Property 12
// Validates: Requirements 8.1, 8.2, 14.6

import { describe, expect } from "vitest";
import { test as testProp, fc } from "@fast-check/vitest";
import { recommendCoolingBreaks } from "../../lib/planner/recommendCoolingBreaks";
import type {
  ScheduleTransition,
  CampusCommitment,
  RouteSegmentRisk,
...
// ---------------------------------------------------------------------------
// Property 12: Valid cooling recommendation fields
// Validates: Requirements 8.1, 8.2, 14.6
// ---------------------------------------------------------------------------

describe("recommendCoolingBreaks – P12: valid cooling recommendation fields", () => {
  // Graph with at least one cooling point, used for property tests
  const graphWithCoolingArb: fc.Arbitrary<{
    graph: CampusGraph;
    transition: ScheduleTransition;
  }> = fc
    .record({
      riskLevel: riskLevelArb,
      coolingLngOffset: fc.float({ min: Math.fround(-0.001), max: Math.fround(0.001), noNaN: true }),
      coolingLatOffset: fc.float({ min: Math.fround(-0.001), max: Math.fround(0.001), noNaN: true }),
    })
    .map(({ riskLevel, coolingLngOffset, coolingLatOffset }) => {
      const b1 = makeNode("b1", "Building 1", "building", 0, 0);
      const b2 = makeNode("b2", "Building 2", "building", 0.001, 0);
      const cp = makeNode(
        "cp1",
        "Test Cooling Station",
        "cooling_point",
        coolingLngOffset,
        coolingLatOffset,
      );
      const edge = makeEdge("b1", "b2");
      const graph = buildTestGraph([b1, b2, cp], [edge]);
      const transition = makeTransition(riskLevel, ["b1", "b2"]);
      return { graph, transition };
    });

  testProp.prop([graphWithCoolingArb], { numRuns: 100 })(
    "returns a recommendation with non-empty name when cooling points exist",
    ({ graph, transition }) => {
      const result = recommendCoolingBreaks(transition, graph);

      expect(result).not.toBeNull();
      expect(result!.coolingPointName).toBeTruthy();
      expect(result!.coolingPointName.length).toBeGreaterThan(0);
    },
  );

  testProp.prop([graphWithCoolingArb], { numRuns: 100 })(
    "suggestedBreakMinutes is between 5 and 15 inclusive",
    ({ graph, transition }) => {
      const result = recommendCoolingBreaks(transition, graph);

      expect(result).not.toBeNull();
      expect(result!.suggestedBreakMinutes).toBeGreaterThanOrEqual(5);
      expect(result!.suggestedBreakMinutes).toBeLessThanOrEqual(15);
    },
  );
  ...
  testProp.prop([riskLevelArb], { numRuns: 100 })(
    "break duration scales with risk level (lower-risk=5, higher-risk=10, not recommended=15)",
    (riskLevel) => {
      const b1 = makeNode("b1", "Building 1", "building", 0, 0);
      const b2 = makeNode("b2", "Building 2", "building", 0.001, 0);
      const cp = makeNode("cp1", "Cooling Station", "cooling_point", 0.0005, 0);
      const graph = buildTestGraph([b1, b2, cp], [makeEdge("b1", "b2")]);
      const transition = makeTransition(riskLevel, ["b1", "b2"]);
      ...
      expect(result).not.toBeNull();
      const expected =
        riskLevel === "lower-risk" ? 5 : riskLevel === "higher-risk" ? 10 : 15;
      expect(result!.suggestedBreakMinutes).toBe(expected);
    },
  );
});

// ---------------------------------------------------------------------------
// Null cases: no cooling points or no route
// ---------------------------------------------------------------------------

describe("recommendCoolingBreaks – null cases", () => {
  testProp.prop([riskLevelArb, segmentRiskArb], { numRuns: 100 })(
    "returns null when graph has no cooling points",
  ...
  testProp.prop([riskLevelArb], { numRuns: 100 })(
    ...
    (riskLevel) => {
      const b1 = makeNode("b1", "Building 1", "building", 0, 0);
      const cp = makeNode("cp1", "Cooling Station", "cooling_point", 0.001, 0);
      const graph = buildTestGraph([b1, cp], []);
